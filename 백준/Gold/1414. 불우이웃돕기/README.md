...
### 주의할 점
모두 연결되었다면, 1~N번째 간선의 부모가 1일 것으로 보고, parent 배열이 총합이 N과 같으면 모두 연결된 것으로 모았으나 틀렸다.
반례는 입력받은 N의 값이 1일때 반드시 parent 배열의 합은 1이된다. 그 자신에게 랜선이 없더라도 말이다. 따라서 N-1개의 간선만큼 연결되었는지 체크하는 방식으로 바꾸어 문제를 해결하였다.
